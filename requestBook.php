...
try {
    ...
    if ($bookData) {
        ...
        $insertQuery = "INSERT INTO tbl_bookinfo_logs 
                (bookname, author, issueddate, returndate, borrowedby, returnedby, bookisinuse, requestby, isrequest, islate, requestdate) 
                VALUES 
                (:bookname, :author, NULL, NULL, NULL, NULL, 0, :requestby, 1, 0, NOW())";
        ...
        // Debugging: Log successful request
        error_log("Book request inserted: " . json_encode([
            'bookname' => $bookName,
            'author' => $author,
            'requestby' => $username
        ]));
